...
tstart = time.time()
df = df.select('atom_from_index','atom_to_index','hop_vec_x','hop_vec_y', 'hop_vec_z', 'ham_real').repartition('atom_from_index', 'atom_to_index')
...
df.select('atom_from_index','atom_to_index','hop_vec_x','hop_vec_y', 'hop_vec_z', 'ham_real').registerTempTable("Test")
sqlContext = SQLContext(sc)
tstart = time.time()
aa = sqlContext.sql("SELECT * FROM Test WHERE atom_from_index = '11' AND atom_to_index = '11'" ).collect()
aa_np = np.array(aa)
...
tstart = time.time()
df.select('atom_from_index','atom_to_index','hop_vec_x','hop_vec_y', 'hop_vec_z', 'ham_real').write.partitionBy('atom_from_index', 'atom_to_index').format("parquet").save("myparquet_short.parquet")
tend = time.time()
print('***************Partitioning took %f second***************'%(tend - tstart))

# Read in the partitioned table
tstart = time.time()
spark.read.format("parquet").load("myparquet_short.parquet").createOrReplaceTempView("fun")
tend = time.time()
